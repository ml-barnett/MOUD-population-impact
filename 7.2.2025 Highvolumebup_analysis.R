...
## Total ARCOS bup prescribed (grams)

df1_arcos <- read_dta("/Users/davidzhu/Downloads/arcos_bup_supply_county_y.dta") %>%
  rename(fips = county_fips) %>%
  mutate(fips = as.character(fips))
head(df1_arcos)

prescriber_df1 <- read_dta("/Users/davidzhu/Downloads/moud_analytic_data_05292025.dta") %>%
  rename(fips = county_fips) %>%
  filter(top5_flag == 1) %>%
  mutate(fips = as.character(fips))

entry_years <- prescriber_df1 %>%
  group_by(fips) %>%
  summarize(first_entry_year = min(firstyear, na.rm = TRUE), .groups = "drop")

exit_years <- prescriber_df1 %>%
  filter(lastyear != 2023) %>%
  group_by(fips) %>%
  summarize(last_exit_year = max(lastyear, na.rm = TRUE), .groups = "drop")

df1_joined <- df1_arcos %>%
  left_join(entry_years, by = "fips") %>%
  left_join(exit_years, by = "fips") %>%
  mutate(
    entry_group = case_when(
      first_entry_year == 2015 ~ "a. 2015",
      first_entry_year == 2016 ~ "b. 2016",
      first_entry_year == 2017 ~ "c. 2017",
      first_entry_year == 2018 ~ "d. 2018",
      first_entry_year == 2019 ~ "e. 2019",
    ...
    exit_group = case_when(
      last_exit_year == 2015 ~ "a. 2015",
      last_exit_year == 2016 ~ "b. 2016",
      last_exit_year == 2017 ~ "c. 2017",
      last_exit_year == 2018 ~ "d. 2018",
      last_exit_year == 2019 ~ "e. 2019",
    ...
    entry_group = ifelse(is.na(entry_group), "i. Never", entry_group),
    exit_group = ifelse(is.na(exit_group), "i. Never", exit_group)
  )

# === Entry Group Counts ===
entry_group_counts_arcos <- df1_joined %>%
  group_by(entry_group) %>%
  summarize(n = n_distinct(fips)) %>%
  arrange(entry_group)
print(entry_group_counts_arcos)

# === Exit Group Counts ===
exit_group_counts_arcos <- df1_joined %>%
  group_by(exit_group) %>%
  summarize(n = n_distinct(fips)) %>%
  arrange(exit_group)
print(exit_group_counts_arcos)

# === PHASE 1 — Entry Group Line Plot ===
avg_by_entry_year <- df1_joined %>%
  group_by(entry_group, year) %>%
  summarize(mean_bup = mean(grams_bup_pharm, na.rm = TRUE), .groups = "drop")

ggplot(avg_by_entry_year, aes(x = year, y = mean_bup, color = entry_group)) +
  geom_line(size = 1.2) +
  scale_x_continuous(breaks = seq(min(avg_by_entry_year$year), max(avg_by_entry_year$year), by = 1)) +
  labs(
    title = "Total Buprenorphine Prescribed (g) by County and Entry Group",
    x = "Year",
    y = "Total Buprenorphine Prescribed (g)",
    color = "Entry Group"
  ) +
  theme_minimal()

# === PHASE 2 — Entry Event-Time Analysis ===
df1_event <- df1_joined %>%
  filter(entry_group != "i. Never" & !is.na(first_entry_year)) %>%
  mutate(event_time = year - first_entry_year) %>%
  filter(event_time >= -3 & event_time <= 3)

event_trends <- df1_event %>%
  group_by(entry_group, event_time) %>%
  summarize(mean_bup = mean(grams_bup_pharm, na.rm = TRUE),
            se = sd(grams_bup_pharm, na.rm = TRUE) / sqrt(n()), .groups = "drop") %>%
  mutate(lower = mean_bup - 1.96 * se, upper = mean_bup + 1.96 * se)

# Line Plot
ggplot(event_trends, aes(x = event_time, y = mean_bup)) +
  geom_line(color = "steelblue", size = 1.2) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "lightblue", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Total Buprenorphine Prescribed (g) by Years from Entry",
       x = "Years from Entry", y = "Total Buprenorphine Prescribed (g)") +
  theme_minimal()

# Bar Plot
ggplot(event_trends, aes(x = factor(event_time), y = mean_bup)) +
  geom_bar(stat = "identity", fill = "steelblue", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = which(levels(factor(event_trends$event_time)) == "0"), linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Total Buprenorphine Prescribed (g) by Years from Entry",
       x = "Years from Entry", y = "Total Buprenorphine Prescribed (g)") +
  theme_minimal()

# === PHASE 3 — Exit Group Line Plot ===
avg_by_exit_year <- df1_joined %>%
  group_by(exit_group, year) %>%
  summarize(mean_bup = mean(grams_bup_pharm, na.rm = TRUE), .groups = "drop")

ggplot(avg_by_exit_year, aes(x = year, y = mean_bup, color = exit_group)) +
  geom_line(size = 1.2) +
  scale_x_continuous(breaks = seq(min(avg_by_exit_year$year), max(avg_by_exit_year$year), by = 1)) +
  labs(
    title = "Total Buprenorphine Prescribed (g) by County and Exit Group",
    x = "Year",
    y = "Total Buprenorphine Prescribed (g)",
    color = "Exit Group"
  ) +
...
# === PHASE 4 — Exit Event-Time Analysis ===
df1_exit <- df1_joined %>%
  filter(exit_group != "i. Never" & !is.na(last_exit_year)) %>%
  mutate(event_time = year - last_exit_year) %>%
  filter(event_time >= -3 & event_time <= 3)

exit_trends <- df1_exit %>%
  group_by(exit_group, event_time) %>%
  summarize(mean_bup = mean(grams_bup_pharm, na.rm = TRUE),
            se = sd(grams_bup_pharm, na.rm = TRUE) / sqrt(n()), .groups = "drop") %>%
  mutate(lower = mean_bup - 1.96 * se, upper = mean_bup + 1.96 * se)

ggplot(exit_trends, aes(x = event_time, y = mean_bup)) +
  geom_line(color = "darkred", size = 1.2) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "salmon", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Total Buprenorphine Prescribed (g) by Years from Exit",
       x = "Years from Exit", y = "Total Buprenorphine Prescribed (g)") +
  theme_minimal()

ggplot(exit_trends, aes(x = factor(event_time), y = mean_bup)) +
  geom_bar(stat = "identity", fill = "darkred", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = which(levels(factor(exit_trends$event_time)) == "0"), linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Total Buprenorphine Prescribed (g) by Years from Exit",
       x = "Years from Exit", y = "Total Buprenorphine Prescribed (g)") +
  theme_minimal()

df2_arcos <- read_dta("/Users/davidzhu/Downloads/arcos_bup_supply_county_y.dta") %>%
  rename(fips = county_fips) %>%
  mutate(fips = as.character(fips))
head(df2_arcos)

# === Load Entry/Exit Metadata from Prescriber Dataset ===
prescriber_df2 <- read_dta("/Users/davidzhu/Downloads/moud_analytic_data_05292025.dta") %>%
  rename(fips = county_fips) %>%
  filter(top5_flag == 1) %>%
  mutate(fips = as.character(fips))

entry_years <- prescriber_df2 %>%
  group_by(fips) %>%
  summarize(first_entry_year = min(firstyear, na.rm = TRUE), .groups = "drop")

exit_years <- prescriber_df2 %>%
  filter(lastyear != 2023) %>%
  group_by(fips) %>%
  summarize(last_exit_year = max(lastyear, na.rm = TRUE), .groups = "drop")

df2_joined <- df2_arcos %>%
  left_join(entry_years, by = "fips") %>%
  left_join(exit_years, by = "fips") %>%
  mutate(
    entry_group = case_when(
...
# === PHASE 1 — Entry Group Line Plot ===
avg_by_entry_year <- df2_joined %>%
  group_by(entry_group, year) %>%
  summarize(mean_bup = mean(bup_per_100000_residents, na.rm = TRUE), .groups = "drop")

ggplot(avg_by_entry_year, aes(x = year, y = mean_bup, color = entry_group)) +
  geom_line(size = 1.2) +
  scale_x_continuous(breaks = seq(min(avg_by_entry_year$year), max(avg_by_entry_year$year), by = 1)) +
  labs(
    title = "Buprenorphine Prescribed (g) per 100,000 Residents by County and Entry Group",
    x = "Year",
    y = "Buprenorphine Prescribed (g) per 100,000 Residents",
    color = "Entry Group"
  ) +
  theme_minimal()

# === PHASE 2 — Entry Event-Time Analysis ===
df2_event <- df2_joined %>%
  filter(entry_group != "i. Never" & !is.na(first_entry_year)) %>%
  mutate(event_time = year - first_entry_year) %>%
  filter(event_time >= -3 & event_time <= 3)

event_trends <- df2_event %>%
  group_by(entry_group, event_time) %>%
  summarize(mean_bup = mean(bup_per_100000_residents, na.rm = TRUE),
            se = sd(bup_per_100000_residents, na.rm = TRUE) / sqrt(n()), .groups = "drop") %>%
  mutate(lower = mean_bup - 1.96 * se, upper = mean_bup + 1.96 * se)
...
ggplot(event_trends, aes(x = event_time, y = mean_bup)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Buprenorphine Prescribed (g) per 100,000 Residents by Years from Entry",
       x = "Years from Entry", y = "BUP per 100,000 Residents") +
  theme_minimal()
...
ggplot(event_trends, aes(x = factor(event_time), y = mean_bup)) +
  ...
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Buprenorphine Prescribed (g) per 100,000 Residents by Years from Entry",
       x = "Years from Entry", y = "Buprenorphine Prescribed (g) per 100,000 Residents") +
  theme_minimal()

# === PHASE 3 — Exit Group Line Plot ===
avg_by_exit_year <- df2_joined %>%
  group_by(exit_group, year) %>%
  summarize(mean_bup = mean(bup_per_100000_residents, na.rm = TRUE), .groups = "drop")
...
ggplot(avg_by_exit_year, aes(x = year, y = mean_bup, color = exit_group)) +
  ...
  scale_x_continuous(breaks = seq(min(avg_by_exit_year$year), max(avg_by_exit_year$year), by = 1)) +
  labs(
    title = "Buprenorphine Prescribed (g) per 100,000 Residents by County and Exit Group",
    x = "Year",
    y = "Buprenorphine Prescribed (g) per 100,000 Residents",
    color = "Exit Group"
  ) +
...
ggplot(exit_trends, aes(x = event_time, y = mean_bup)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Buprenorphine Prescribed (g) per 100,000 Residents by Years from Exit",
       x = "Years from Exit", y = "Buprenorphine Prescribed (g) per 100,000 Residents") +
  theme_minimal()
...
##Code for first and last prescriber in that county
...
# Read data
df <- read_dta("/Users/davidzhu/Downloads/moud_analytic_data_05292025.dta")
df <- df %>% mutate(county_fips = as.character(county_fips))  # enforce character early
head(df)

# FIRST PRESCRIBER COHORTS
first_entry_years <- df %>%
  group_by(county_fips) %>%
  summarize(first_prescriber_year = min(firstyear, na.rm = TRUE), .groups = "drop") %>%
  mutate(entry_group = case_when(
    first_prescriber_year %in% 2013:2015 ~ "a. 2013-2015",
    first_prescriber_year == 2016 ~ "b. 2016",
    first_prescriber_year == 2017 ~ "c. 2017",
    first_prescriber_year == 2018 ~ "d. 2018",
    first_prescriber_year == 2019 ~ "e. 2019",
    first_prescriber_year == 2020 ~ "f. 2020",
    first_prescriber_year == 2021 ~ "g. 2021",
    first_prescriber_year %in% 2022:2023 ~ "h. 2022-2023",
    TRUE ~ NA_character_
  ))

first_entry_years <- df %>% distinct(county_fips) %>%
  left_join(first_entry_years, by = "county_fips") %>%
  mutate(entry_group = ifelse(is.na(entry_group), "i. Never", entry_group))

# LAST PRESCRIBER COHORTS
last_exit_years <- df %>%
  filter(lastyear != 2023) %>%
  group_by(county_fips) %>%
  summarize(last_prescriber_year = max(lastyear, na.rm = TRUE), .groups = "drop") %>%
  mutate(exit_group = case_when(
    last_prescriber_year %in% 2013:2015 ~ "a. 2013-2015",
    last_prescriber_year == 2016 ~ "b. 2016",
    last_prescriber_year == 2017 ~ "c. 2017",
    last_prescriber_year == 2018 ~ "d. 2018",
    last_prescriber_year == 2019 ~ "e. 2019",
    last_prescriber_year == 2020 ~ "f. 2020",
    last_prescriber_year == 2021 ~ "g. 2021",
    last_prescriber_year == 2022 ~ "h. 2022",
    TRUE ~ NA_character_
  ))

last_exit_years <- df %>% distinct(county_fips) %>%
  left_join(last_exit_years, by = "county_fips") %>%
  mutate(exit_group = ifelse(is.na(exit_group), "i. Never or Still Active", exit_group))

# EVENT TIME FOR FIRST PRESCRIBER
df_first_grouped <- df %>%
  left_join(first_entry_years, by = "county_fips") %>%
  mutate(event_time = year - first_prescriber_year)

df_first_window <- df_first_grouped %>%
  filter(!entry_group %in% c("a. 2013-2015", "h. 2022-2023"),
         event_time >= -3, event_time <= 3)

first_event_trends <- df_first_window %>%
  group_by(entry_group, event_time) %>%
  summarize(mean_claims = mean(Tot_Clms, na.rm = TRUE),
            se = sd(Tot_Clms, na.rm = TRUE) / sqrt(n()), .groups = "drop") %>%
  mutate(lower = mean_claims - 1.96 * se,
         upper = mean_claims + 1.96 * se)

# LINE + BAR PLOTS
ggplot(first_event_trends, aes(x = event_time, y = mean_claims)) +
  geom_line(color = "steelblue", size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "lightblue", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Buprenorphine Claims by Years from First Prescriber Entry",
       x = "Years from Entry", y = "Average Number of Claims") +
  theme_minimal()

ggplot(first_event_trends, aes(x = factor(event_time), y = mean_claims)) +
  geom_bar(stat = "identity", fill = "steelblue", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = which(levels(factor(first_event_trends$event_time)) == "0"),
             linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Buprenorphine Claims by Years from First Prescriber Entry",
       x = "Years from Entry", y = "Average Number of Claims") +
...
# EVENT TIME FOR LAST PRESCRIBER
df_last_grouped <- df %>%
  left_join(last_exit_years, by = "county_fips") %>%
  mutate(event_time = year - last_prescriber_year)

df_last_window <- df_last_grouped %>%
  filter(!exit_group %in% c("a. 2013-2015"),
         event_time >= -3, event_time <= 3)

last_event_trends <- df_last_window %>%
  group_by(exit_group, event_time) %>%
  summarize(mean_claims = mean(Tot_Clms, na.rm = TRUE),
            se = sd(Tot_Clms, na.rm = TRUE) / sqrt(n()), .groups = "drop") %>%
  mutate(lower = mean_claims - 1.96 * se,
...
# LINE + BAR PLOTS
ggplot(last_event_trends, aes(x = event_time, y = mean_claims)) +
  geom_line(color = "darkred", size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "salmon", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Buprenorphine Claims by Years from Last Prescriber Exit",
       x = "Years from Exit", y = "Average Number of Claims") +
  theme_minimal()

ggplot(last_event_trends, aes(x = factor(event_time), y = mean_claims)) +
  geom_bar(stat = "identity", fill = "darkred", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = which(levels(factor(last_event_trends$event_time)) == "0"),
             linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Buprenorphine Claims by Years from Last Prescriber Exit",
       x = "Years from Exit", y = "Average Number of Claims") +
...
# === Load ARCOS data ===
df_arcos <- read_dta("/Users/davidzhu/Downloads/arcos_bup_supply_county_y.dta") %>%
  rename(county_fips = county_fips) %>%
  mutate(county_fips = as.character(county_fips))

# === Load first_prescriber_flag / last_prescriber_flag metadata ===
df_flags <- read_dta("/Users/davidzhu/Downloads/moud_analytic_data_05292025.dta") %>%
  rename(county_fips = county_fips) %>%
  mutate(county_fips = as.character(county_fips))

# FIRST PRESCRIBER EVENT TIME
first_entry_years_flags <- df_flags %>%
  group_by(county_fips) %>%
  summarize(first_prescriber_year = min(firstyear, na.rm = TRUE), .groups = "drop") %>%
  mutate(entry_group = case_when(
    first_prescriber_year %in% 2013:2015 ~ "a. 2013-2015",
...
first_entry_years_flags <- df_flags %>% distinct(county_fips) %>%
  left_join(first_entry_years_flags, by = "county_fips") %>%
  mutate(entry_group = ifelse(is.na(entry_group), "i. Never", entry_group))

# LAST PRESCRIBER EVENT TIME
last_exit_years_flags <- df_flags %>%
  filter(lastyear != 2023) %>%
  group_by(county_fips) %>%
  summarize(last_prescriber_year = max(lastyear, na.rm = TRUE), .groups = "drop") %>%
  mutate(exit_group = case_when(
...
last_exit_years_flags <- df_flags %>% distinct(county_fips) %>%
  left_join(last_exit_years_flags, by = "county_fips") %>%
  mutate(exit_group = ifelse(is.na(exit_group), "i. Never or Still Active", exit_group))

# === JOIN TO ARCOS ===
df_arcos_joined <- df_arcos %>%
  left_join(first_entry_years_flags, by = "county_fips") %>%
  left_join(last_exit_years_flags, by = "county_fips")

# === EVENT TIME for FIRST PRESCRIBER ===
df_arcos_first <- df_arcos_joined %>%
  mutate(event_time = year - first_prescriber_year) %>%
  filter(!entry_group %in% c("a. 2013-2015", "h. 2022-2023"),
         event_time >= -3, event_time <= 3)

first_event_trends_arcos <- df_arcos_first %>%
  group_by(entry_group, event_time) %>%
  summarize(mean_bup = mean(bup_per_100000_residents, na.rm = TRUE),
            se = sd(bup_per_100000_residents, na.rm = TRUE) / sqrt(n()),
            .groups = "drop") %>%
  mutate(lower = mean_bup - 1.96 * se,
         upper = mean_bup + 1.96 * se)

# LINE + BAR plots for FIRST PRESCRIBER
ggplot(first_event_trends_arcos, aes(x = event_time, y = mean_bup)) +
  geom_line(color = "steelblue", size = 1.2) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "lightblue", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Buprenorphine (g/100k) by Years from First Prescriber Entry",
       x = "Years from Entry", y = "Buprenorphine (g per 100k)") +
  theme_minimal()

ggplot(first_event_trends_arcos, aes(x = factor(event_time), y = mean_bup)) +
  geom_bar(stat = "identity", fill = "steelblue", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = which(levels(factor(first_event_trends_arcos$event_time)) == "0"), linetype = "dashed") +
  facet_wrap(~entry_group, scales = "free_y") +
  labs(title = "Buprenorphine (grams per 100,00) by Years from First Prescriber Entry",
       x = "Years from Entry", y = "Buprenorphine (g per 100k)") +
  theme_minimal()

# === EVENT TIME for LAST PRESCRIBER ===
df_arcos_last <- df_arcos_joined %>%
  mutate(event_time = year - last_prescriber_year) %>%
  filter(!exit_group %in% c("a. 2013-2015"),
         event_time >= -3, event_time <= 3)

last_event_trends_arcos <- df_arcos_last %>%
  group_by(exit_group, event_time) %>%
  summarize(mean_bup = mean(bup_per_100000_residents, na.rm = TRUE),
            se = sd(bup_per_100000_residents, na.rm = TRUE) / sqrt(n()),
            .groups = "drop") %>%
...
# LINE + BAR plots for LAST PRESCRIBER
ggplot(last_event_trends_arcos, aes(x = event_time, y = mean_bup)) +
  geom_line(color = "darkred", size = 1.2) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "salmon", alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Buprenorphine (grams per 100,00) by Years from Last Prescriber Exit",
       x = "Years from Exit", y = "Buprenorphine (g per 100k)") +
  theme_minimal()

ggplot(last_event_trends_arcos, aes(x = factor(event_time), y = mean_bup)) +
  geom_bar(stat = "identity", fill = "darkred", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = which(levels(factor(last_event_trends_arcos$event_time)) == "0"), linetype = "dashed") +
  facet_wrap(~exit_group, scales = "free_y") +
  labs(title = "Buprenorphine (g/100k) by Years from Last Prescriber Exit",
       x = "Years from Exit", y = "Buprenorphine (g per 100k)") +
  theme_minimal()
